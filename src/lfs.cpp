...
lfs::batch_response lfs::create_batch_response(const std::string& operation,
                                               const json_array_t& objects,
                                               const server::config::data& cfg,
                                               const std::string& authorization,
                                               lfs::log& logger) {
  static const error_object error{
  ...
  if (operation == "download") {
    for (const auto& o : objects) {
      ...
      if (oid.empty() ||
          !utils::can_open(get_filesystem_path(cfg.file_directory, oid))) {
        br.objects.push_back({bo, {}, {}, error});
        continue;
      }

      // If the size does not come in the request or they specified 0, obtain
      // it.
      if (size == 0) {
        bo.size = utils::get_file_size(
            get_filesystem_path(cfg.file_directory, oid), logger);
      }

      operation_object actions = {
          {authorization},  // Header.
          get_href(cfg.scheme, cfg.host, std::to_string(cfg.port),
                   cfg.download_object_path, bo.oid),
      };
    ...
    for (const auto& o : objects) {
      batch_object bo{o["oid"].string_value(), o["size"].int_value()};

      operation_object actions = {
          {authorization},  // Header.
          get_href(cfg.scheme, cfg.host, std::to_string(cfg.port),
                   cfg.upload_object_path, bo.oid),
      };
...
void lfs::download_handler(const request_t& request, response_t& response,
  ...
  std::ifstream file(get_filesystem_path(cfg.file_directory, oid),
...
// Git format.
std::string lfs::get_filesystem_path(const std::string& file_directory,
...
void lfs::upload_handler(const request_t& request, response_t& response,
                         const server::config::data& cfg, lfs::log& logger) {
...
void lfs::save_file_in_directory(const std::string& oid, const std::string& raw,
                                 const server::config::data& cfg,
                                 lfs::log& logger) {
  const auto oid_data = split_oid(oid);

  if (!utils::directory_exists(cfg.file_directory + '/' +
                               oid_data.parent_dir)) {
    if (utils::create_directory(cfg.file_directory + '/' +
  ...
  if (!utils::directory_exists(cfg.file_directory + '/' + oid_data.parent_dir +
                               '/' + oid_data.child_dir)) {
    if (utils::create_directory(cfg.file_directory + '/' + oid_data.parent_dir +
  ...
  std::ofstream file{get_filesystem_path(cfg.file_directory, oid),
...
bool lfs::process_auth(const request_t& request, response_t& response,
  ...
  if (!parse_auth(request, credentials) || !authenticate(credentials, cfg)) {
    response.status =
        static_cast<int>(http_response_codes::auth_required);
...
bool lfs::authenticate(const user_data& credentials,
                       const server::config::data& cfg) {
...
// PREFIX is Basic.
lfs::user_data lfs::parse_b64_auth(const std::string& auth,
  ...
  return {decoded_auth.substr(0, colon_pos),
          decoded_auth.substr(colon_pos + 1)};
}
